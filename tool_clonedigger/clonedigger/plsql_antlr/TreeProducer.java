...
public static void main (String[]args) throws Exception
{
    ...
    ParseTree tree = parser.compilation_unit(); // parse


    // Er zijn twee methodes om een XML AST te creeren.
    // 1. Via de TreePrinterListener (nieuw - niet gebruikt)
    TreePrinterListener listener = new TreePrinterListener(parser);
	ParseTreeWalker.DEFAULT.walk(listener, tree);
	String formatted = listener.toString();
	PrintWriter outputStreamNew = new PrintWriter (new FileWriter (args[1], false));
    outputStreamNew.println("<?xml version=\"1.0\" ?>");
    outputStreamNew.println(formatted);
    outputStreamNew.close();


	// 2. Via de bestaandde printTree optie (oud)
    PrintWriter outputStream = new PrintWriter (new FileWriter (args[1]+"old.xml", false));
