...
class PlsqlANTLRSourceFile (SourceFile):
    extension = 'sql'
    size_threshold = 3
    ...
    def __init__(self, file_name):
        ...
        class ExpatHandler:
            ...
            def start_element(expat_self, xml_node_name, attrs):
                ...
                if name in ["compilation_unit", "unit_statement", "select_statement", "query_block"]:
                    r.markAsStatement()
                else:
        ...
        tree_producer_path = 'TreeProducer.jar'
        ...
        if os.name in ['mac', 'posix']:
            ...
            assert(0)
        if os.system('java -jar ' + tree_producer_path + ' %s %s 2>err.log' %(file_name, tree_file_name)):
            print 'Location: '
            print os.getcwd()
            f = open('err.log')
            s = f.read()
            f.close()
            raise Exception(s + 'java -jar ' + tree_producer_path + ' %s %s 2>err.log' %(file_name, tree_file_name))
